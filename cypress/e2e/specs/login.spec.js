describe('Login Page', () => {
  ...
  it('shows Login with SSO button', () => {
    cy.mockPrivatePortal({}, { oidcAuthEnabled: true, basicAuthEnabled: false })
  ...
  it('does not show Login with SSO button', () => {
    cy.mockPrivatePortal({}, { oidcAuthEnabled: false })
  ...
  it('routes to intended page after SSO login', () => {
    cy.intercept('/kauth/api/v1/developer-authenticate/123*', (req) => {
      req.redirect('/login?loginSuccess=true', 301)
    })

    cy.mockGetUserInfo()

    cy.mockPrivatePortal({}, { oidcAuthEnabled: true })
