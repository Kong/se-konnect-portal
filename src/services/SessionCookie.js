...
export default class SessionCookie {
  ...
  async saveData (data, force = true) {
  ...
  async destroy (to) {
    ...
    try {
      ...
      const response = await kongAuthApi.authentication.logoutDeveloper()

      const { data: { loginPath, singleLogoutURL, singleLogoutEnabled } } = response
      ...
      // If singleLogoutEnabled, return IdP logout URL
      if (singleLogoutEnabled && singleLogoutURL) {
        // Build new URL from singleLogoutUrl
        const logoutUrl = new URL(singleLogoutURL)

        // Add the Okta-specific `fromURI` parameter
        logoutUrl.searchParams.append('fromURI', `${window.location.origin}/${loginPagePath}`)
  ...
  async refreshToken () {
    ...
    try {
      // Trigger auth refresh
      const response = await kongAuthApi.authentication.refreshDeveloper({})
